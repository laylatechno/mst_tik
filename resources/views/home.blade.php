    ...
    <div class="container-fluid">
        ...
        <div class="row align-items-center">
            <div class="col-lg-12 col-md-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Kunjungan Halaman Utama</h5>
                        <canvas id="visitorHomeChart"></canvas>
                    </div>
                </div>
            </div>
        </div>
         ...
         @endcan
        @can('dashboard-content-store')
            <div class="row align-items-center">
                <div class="col-lg-12 col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Kunjungan Halaman Pengguna</h5>
                            <canvas id="visitorUserChart"></canvas>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <form action="{{ route('home') }}" method="GET">
                                    <div class="row">
                                        ...
                                        <div class="col-12 mb-3">
                                            ...
                                            <div>
                                                <input type="radio" id="filter_month" name="filter_type" value="month"
                                                    onchange="toggleFilterOptions('month')"
                                                    {{ request('filter_type') == 'month' ? 'checked' : '' }}>
                                                <label for="filter_month">Bulan</label>

                                                <input type="radio" id="filter_date" name="filter_type" value="date"
                                                    onchange="toggleFilterOptions('date')"
                                                    {{ request('filter_type') == 'date' ? 'checked' : '' }}>
                                                <label for="filter_date">Tanggal</label>
                                            </div>
                                        </div>

                                        <!-- Filter berdasarkan Bulan -->
                                        <div class="col-6 mb-3" id="month_filter"
                                        ...
                                        <div class="col-3 mb-3" id="date_filter_start"
                                            style="display: {{ request('filter_type') == 'date' ? 'block' : 'none' }}">
                                            <label for="start_date" class="form-label">Tanggal Mulai</label>
                                            <input type="date" class="form-control" id="start_date" name="start_date"
                                                value="{{ request('start_date') }}">
                                        </div>
                                        <div class="col-3 mb-3" id="date_filter_end"
                                            style="display: {{ request('filter_type') == 'date' ? 'block' : 'none' }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-6 col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title text-center mb-4">Grafik Transaksi Berdasarkan Kategori
                                            {{ $currentYear }}</h5>
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title text-center mb-4">Grafik Transaksi Berdasarkan Kas
                                            {{ $currentYear }}</h5>
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <h6 class="fw-semibold text-dark fs-4 mb-0">Rp.
                                                {{ number_format($totalMonthlyPurchases, 0, ',', '.') }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <h6 class="fw-semibold text-dark fs-4 mb-0">Rp.
                                                {{ number_format($totalMonthlyOrders, 0, ',', '.') }}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card bg-primary border-0 w-100">
                        <div class="card-body pb-0">
                            ...
                            <div class="text-center mt-0">
                                <img src="{{ asset('template/back') }}/dist/images/backgrounds/piggy.png" class="img-fluid"
                                    alt="" />
                            </div>
                        </div>
                                ...
                                @foreach ($bestSellingProducts as $index => $product)
                                    <div class="mb-{{ $loop->last ? '0' : '3' }} pb-{{ $loop->last ? '0' : '1' }}">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <div>
                                                <span
                                                    class="badge {{ $index < 2 ? 'bg-light-primary text-primary' : 'bg-light-secondary text-secondary' }} fw-semibold fs-3">
                                                    {{ $product->percentage }}%
                                                </span>
                                            </div>
                                        </div>
                                        <div class="progress {{ $index < 2 ? 'bg-light-primary' : 'bg-light-secondary' }}"
                                            style="height: 4px;">
                                            <div class="progress-bar {{ $index >= 2 ? 'bg-secondary' : '' }}"
                                                style="width: {{ $product->percentage }}%" role="progressbar"
                        ...
                        <div class="card-body">
                            ...
                            <div class="position-relative">
                                @foreach ($topCustomers as $index => $customer)
                                    ...
                                    @endphp
                                    <div
                                        class="d-flex align-items-center justify-content-between {{ !$loop->last ? 'mb-7' : '' }}">
                                        <div class="d-flex">
                                            <div
                                                class="p-6 {{ $colors[$index]['bg'] }} rounded me-6 d-flex align-items-center justify-content-center">
                                        ...
                                        <div class="{{ $colors[$index]['bg'] }} badge">
                                            <p class="fs-3 {{ $colors[$index]['text'] }} fw-semibold mb-0">
                                                    ...
                                                    <div class="d-flex align-items-center">
                                                        <div class="me-2 pe-1">
                                                            ...
                                                            @else
                                                                ...
                                                                <img src="{{ asset('upload/products/default.png') }}"
                                                                    class="rounded-circle" width="40" height="40"
                                                                    alt="Default Image" />
                                                            @endif
                                                        </div>
...
@push('script')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Home page visits chart (cek apakah elemen ada sebelum mengaksesnya)
            const homeVisitCanvas = document.getElementById('visitorHomeChart');
            if (homeVisitCanvas) {
                const homeVisitCtx = homeVisitCanvas.getContext('2d');
                const homeVisitChart = new Chart(homeVisitCtx, {
                    type: 'line',
                    data: {
                        labels: [
                            @foreach ($homePageVisits as $visit)
                                '{{ \Carbon\Carbon::parse($visit->date)->format('d M') }}',
                            @endforeach
                        ],
                        datasets: [{
            ...
            // User page visits chart (pastikan tetap dirender)
            const userVisitCanvas = document.getElementById('visitorUserChart');
            if (userVisitCanvas) {
                const userVisitCtx = userVisitCanvas.getContext('2d');

                // Generate a color for each user
                const generateColors = (index, alpha) => {
                    const colors = [
                        ...
                        `rgba(210, 199, 100, ${alpha})`
                    ];
                    return colors[index % colors.length];
                };

                // Get all dates for the x-axis
                const allDates = @json($homePageVisits->pluck('date')->toArray());

                // Create datasets for each user
                const userDatasets = @json($uniqueUsers).map((username, index) => {
                    return {
                        label: username,
                        data: allDates.map(date => {
                            const userVisits = @json($userVisitsByDate);
                            return userVisits[date] && userVisits[date][username] ? userVisits[date]
                                [
                                    username
                                ] : 0;
                        }),
                        backgroundColor: generateColors(index, 0.2),
                ...
                const userVisitChart = new Chart(userVisitCtx, {
                    type: 'line',
                    data: {
                            ...
                            tooltip: {
                                callbacks: {
                                    ...
                                    label: function(context) {
                                        return `${context.dataset.label}: ${context.raw} kunjungan`;
                                    }
                                }
                            }
                        }
