                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-3 mt-3">
                                                ...
                                                <select name="status" id="status" class="form-select">
                                                    ...
                                                    <option value="Pesanan Pembelian" {{ request('status') == 'Pesanan Pembelian' ? 'selected' : '' }}>
                                                        Pesanan Pembelian
                                                    </option>
                                            ...
                                            <!-- Kolom Filter Supplier -->
                                            <div class="col-md-3 mt-3">
                                                <label for="supplier_id" class="form-label">Supplier</label>
                                                <select name="supplier_id[]" id="supplier_id" class="form-select" multiple>
                                                    @foreach ($suppliers as $supplier)
                                                    <option value="{{ $supplier->id }}"
                                                        {{ (is_array(request('supplier_id')) && in_array($supplier->id, request('supplier_id'))) ? 'selected' : '' }}>
                                                        {{ $supplier->name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="row mt-3">
                                            <div class="col">
                                                <div class="d-flex flex-wrap gap-2">
                                                    ...
                                                    <!-- Tombol Reset -->
                                                    <a href="{{ route('report.purchase_reports') }}" class="btn btn-secondary">
                                                        <i class="fa fa-sync"></i> Reset
                                                    </a>

                                                    <!-- Tombol Export Excel -->
                                                    <a href="{{ route('report.purchase_reports.export', [
                                                    ...
                                                    <!-- Tombol Export PDF -->
                                                    <a href="{{ route('report.purchase_reports.export_pdf', [
                                                        'start_date' => request('start_date', now()->format('Y-m-d')),
                                                        'end_date' => request('end_date', now()->format('Y-m-d')),
                                                        'status' => request('status'),
                                                        'supplier_id' => request('supplier_id'),
                                                    ...
                                                    <!-- Tombol Preview PDF -->
                                                    <a href="{{ route('report.purchase_reports.preview_pdf', [
                                                        'start_date' => request('start_date', now()->format('Y-m-d')),
                                                        'end_date' => request('end_date', now()->format('Y-m-d')),
                                                        'status' => request('status'),
                                                        'supplier_id' => request('supplier_id'),
