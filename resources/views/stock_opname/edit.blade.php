            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('stock_opname.update', $data_stock_opname->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea name="description" id="description" class="form-control">{{ old('description', $data_stock_opname->description) }}</textarea>
                            </div>
                            ...
                            <div class="form-group mb-3" id="image_container">
                                ...
                                <img id="preview_image" src="{{ asset('upload/stock_opname/'.$data_stock_opname->image) }}" alt="Preview Image" style="max-width: 30%; margin-top: 10px;">
                                @else
                            ...
                            <table id="scroll_hor" class="table border table-striped table-bordered display nowrap" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" name="products[{{ $product->id }}][physical_stock]" class="form-control"
                                                value="{{ old("products.{$product->id}.physical_stock", $data_stock_opname->stockOpnameDetails->firstWhere('product_id', $product->id)->physical_stock ?? 0) }}">
                                        </td>
                                        <td>
                                            <input type="number" name="products[{{ $product->id }}][difference]" class="form-control"
                                                value="{{ old("products.{$product->id}.difference", $data_stock_opname->stockOpnameDetails->firstWhere('product_id', $product->id)->difference ?? 0) }}" readonly>
                                        </td>
                                        <td>
                                            <input type="text" name="products[{{ $product->id }}][description_detail]" class="form-control"
                                                value="{{ old("products.{$product->id}.description_detail", $data_stock_opname->stockOpnameDetails->firstWhere('product_id', $product->id)->description_detail ?? '') }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
